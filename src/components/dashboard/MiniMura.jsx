...
export function MiniMura({ muraIngresos, muraGastos, muraProfit, muraProfitMargen, loading }) {
  ...
  if (muraIngresos === 0 && muraGastos === 0) {
    return (
      <Card>
        <CardHeader><CardTitle>Mura Café — P&L del mes</CardTitle><Coffee size={16} className="text-mura-500" /></CardHeader>
        <div className="text-center py-6 text-sm text-gray-400 dark:text-gray-500">Sin movimientos este mes</div>
      </Card>
    )
  }
  ...
  return (
    <Card>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Ingresos', value: muraIngresos, color: 'text-brand-600 dark:text-brand-400' },
          { label: 'Gastos', value: muraGastos, color: 'text-red-500' },
          { label: 'Profit', value: muraProfit, color: isPositive ? 'text-brand-600 dark:text-brand-400' : 'text-red-500' },
        ].map(({ label, value, color }) => (
          <div key={label} className="text-center">
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">{label}</p>
            <p className={cn('text-sm font-bold tabular-nums', color)}>{formatCOP(value)}</p>
          </div>
        ))}
      </div>
