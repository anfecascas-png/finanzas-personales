            ...
            <tr key={row.ID || i} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
              ...
              <td className="py-3 px-2">
                <div className="flex flex-wrap gap-1">
                  ...
                  {parseFrecuencia(row.Notas) && (
                    <Badge variant={FRECUENCIA_BADGE[parseFrecuencia(row.Notas)] || 'gray'}>
                      {parseFrecuencia(row.Notas)}
                    </Badge>
                  )}
                </div>
              </td>
              <td className="py-3 px-2 text-right font-semibold text-red-600 dark:text-red-400 tabular-nums">
                {formatCOP(row.Monto)}
              </td>
              <td className="py-3 px-2">
                <button
